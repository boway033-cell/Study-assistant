...
1. 增加 `OCR_WORKERS`，默认值为 1（安全档；2~4 为实验性并发，未完成真实基准前不默认开启）。
...
```env
# 默认 1（安全档）。2~4 为实验性并发，未完成真实基准前请勿默认开启。
OCR_WORKERS=1
OCR_RENDER_AHEAD=3
...
的全量在全新 basetemp 下为 280 passed / 1 skipped，PPT 测试未再触发该守卫。100 页真实
扫描件 1/2/3 worker 基准**仍未完成**（见 §5.4 验收表「⏳ 未做」与 §9 方案）。
