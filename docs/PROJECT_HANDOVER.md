...
- **本地能力（无 AI 也完整）**：**连续/单页/双页三种显示模式**（工具栏切换，双页自动适配）、翻页/缩放/适应宽/适应页、**目录侧边栏跳转**、**深色模式**、**阅读位置记忆**（localStorage，知识树跳转时不被覆盖）、**四色高亮 + 批注卡片**（黄/绿/蓝/粉、笔记、挂知识树节点、点击编辑/删除、**Markdown 导出**）、页码指示=视口顶部页
- **AI 可选增强（无 Key 优雅降级）**：选中文字 → 💡解释/🌐翻译（DeepSeek，结果可**一键存为高亮批注**）；📝总结本章（DeepSeek）；🤖解读本页 → **Qwen-VL 视觉分析**（qwen3-vl-plus，Key 已在 .env，仅本机）
