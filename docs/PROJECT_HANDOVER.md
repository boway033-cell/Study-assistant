...
> **最后更新**：综合研读自主分析、知识沉淀实体解耦与目录编号链修正
...
- **自定义协议（已注册）**：当前用户已注册 `study-assistant://open`；支持如 `study-assistant://open/reader/6?page=21` 的白名单深链。WMI 启动独立 `pythonw + scripts/runtime/server_runner.py`，协议处理器退出后服务仍存活；`scripts/protocol/uninstall.ps1` 可撤销。
