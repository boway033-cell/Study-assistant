...
2. 整篇或多章节证据按顶层章节/SI 分组，以平方根权重分配字符预算，组内均匀覆盖前、中、后部，同时输出内容与结构覆盖率。
3. 本地路由先判定 discovery / methods / resource / clinical / materials / review。
4. 提纲遵循“问题—缺口—方法—证据—可信度—意义—边界”，每个事实页绑定 source ID；先停在 `outline_ready` 供人工编辑，再显式提交渲染。
5. 主张—来源审计不使用模型的“自证”作为真值，而是保守检查来源存在性、数字一致性、词项重叠与可定位性。
6. 有模型时只发送所选片段；无模型时生成本地证据提纲，不补造研究结果。
7. 输出 16:9 可编辑 PPTX、来源备注和 manifest；PowerPoint 可用时实际渲染 PNG 预览并做像素级版本差异。

## SI 与来源权利

- `literature_resources` 把主文、SI、图、表和数据集建模为文献的关联资源，可链接到另一本已导入资料。
- 权利字段包含 SPDX 式许可证表达、RightsStatements URI、权利状态、权利人、归属文字、授权说明、是否可复用和 provenance JSON。
- 个人/内部使用中，权利不明内容需明确确认风险；公开/商业输出中，权利不明图像直接被排除，不把“能下载”等同于“可复用”。
