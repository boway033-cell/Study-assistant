<template>
  <div class="knowledge-inbox">
    ...
    <template v-else>
      ...
      <el-pagination v-if="total>filters.page_size" class="record-pagination" v-model:current-page="filters.page" :page-size="filters.page_size" :total="total" layout="prev, pager, next, total" @current-change="loadRecords" />
    </template>
    ...
    <el-drawer v-model="reader.visible" :title="reader.note?.title || '知识笔记'" size="100%" destroy-on-close class="immersive-note-drawer">
      <div class="note-reader" v-loading="reader.loading">
        <template v-if="reader.note">
          ...
          <div v-if="reader.note.source_refs?.length" class="source-ref-list"><b>报告引用回链</b><button v-for="source in reader.note.source_refs.slice(0,20)" :key="String(source)" @click="openSourceRef(source)">{{ typeof source==='string'?source:'来源记录' }} ↗</button></div>
...
import { listKnowledgeRecords, promoteAnnotation, createKnowledgeNote, getKnowledgeNote, updateKnowledgeNote, addKnowledgeNoteToTree, createEvidenceCard, getEvidenceCard, updateEvidenceCard, getBook } from '../api'
defineProps({ embedded: { type: Boolean, default: false } })
const router=useRouter(), loading=ref(false), records=ref([]), chapters=ref([]), selectedKeys=ref([]), total=ref(0)
const COLORS=[{label:'黄色',value:'#f9e572'},{label:'绿色',value:'#9be5a0'},{label:'蓝色',value:'#8ec8f5'},{label:'粉色',value:'#f5b8c8'}]
const filters=reactive({q:'',record_types:['highlight','underline','annotation','note','evidence'],chapter_id:null,color:null,tag:'',dates:null,page:1,page_size:50})
...
const loadRecords=async()=>{loading.value=true;try{const params={book_ids:knowledgeBookIds.value,record_types:filters.record_types,q:filters.q||undefined,chapter_id:filters.chapter_id||undefined,color:filters.color||undefined,tag:filters.tag||undefined,page:filters.page,page_size:filters.page_size};if(filters.dates?.length){params.date_from=filters.dates[0].toISOString();const end=new Date(filters.dates[1]);end.setHours(23,59,59,999);params.date_to=end.toISOString()}const result=await listKnowledgeRecords(params);records.value=result.items||[];total.value=result.total||0;selectedKeys.value=selectedKeys.value.filter(key=>records.value.some(r=>r.key===key))}catch(e){ElMessage.error(`无法加载知识记录：${e.message}`)}finally{loading.value=false}}
...
const openSourceRef=source=>{const match=String(typeof source==='string'?source:source?.ref||'').match(/^B(\d+)(?::CH\d+)?(?::P(\d+)(?:-\d+)?)?/);if(!match)return ElMessage.warning('该来源记录没有可定位页码');router.push({path:`/reader/${match[1]}`,query:match[2]?{page:match[2]}:{}})}
...
<style scoped>
.record-pagination{justify-content:flex-end;margin-top:14px}
.record-grid{grid-template-columns:1fr!important;gap:1px!important;overflow:hidden;border:1px solid var(--study-card-border);border-radius:10px;background:var(--study-card-border)}
.record-card{min-height:0!important;border:0!important;border-radius:0!important;box-shadow:none!important;padding:11px 13px!important;transform:none!important}
.record-card:hover{background:#fffaf2;box-shadow:none!important;transform:none!important}
.record-card h3{margin:7px 0 4px!important;font-size:15px!important}.record-card footer{padding-top:7px!important}
.source-ref-list{display:flex;flex-wrap:wrap;gap:6px;margin-top:12px;padding:10px;border:1px solid var(--study-card-border);border-radius:8px;background:var(--study-surface-muted)}.source-ref-list b{width:100%;font-size:12px}.source-ref-list button{padding:3px 7px;border:1px solid var(--study-card-border);border-radius:6px;background:var(--study-surface-paper);color:var(--el-color-primary);cursor:pointer}
...
.note-reader{width:min(100%,1080px);min-height:calc(100dvh - 150px);margin:0 auto}.note-reader-content{width:min(100%,76ch);margin:0 auto;padding:28px 6px 72px;font-size:17px;line-height:1.9}
</style>
