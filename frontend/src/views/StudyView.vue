...
import { getBook, getTask, studyOverview, studyReports, listKnowledgeRecords, createKnowledgeNote, createEvidenceCard } from '../api'
...
const loadReports = async () => { try { reports.value = await studyReports() } catch (error) { ElMessage.error(error.message) } }
const openReport = report => { activeReport.value = report; content.value = report.content; focus.value = report.focus || focus.value; framework.value = report.framework || framework.value; researchMode.value = report.research_mode || report.selection?.research_mode || 'adaptive'; reasoningDepth.value = report.reasoning_depth || report.selection?.reasoning_depth || 'standard'; selectedChapterIds.value = report.selection?.chapter_ids || []; selectedNoteIds.value = report.selection?.note_ids || [] }
const generate = async () => { if (!focus.value.trim()) return ElMessage.warning('请先写明研究问题'); loading.value = true; progress.value = 0; stage.value = '正在提交到全局任务中心'; try { const response = await studyOverview({ book_ids: knowledgeBookIds.value, chapter_ids: selectedChapterIds.value, note_ids: selectedNoteIds.value, focus: focus.value.trim(), framework: framework.value.trim(), research_mode: researchMode.value, reasoning_depth: reasoningDepth.value }); notifyTaskSubmitted(); for (let i=0; i<180; i++) { await new Promise(resolve => setTimeout(resolve, 2000)); const task = await getTask(response.task_id); progress.value = Math.round((task.progress || 0) * 100); stage.value = task.message || task.stage || '处理中'; if (task.status === 'done') { await loadReports(); const latest = reports.value.find(report => report.id === task.result?.report_id) || scopedReports.value[0]; if (latest) openReport(latest); ElMessage.success('研究报告、研究路径与主张审计已完成'); return } if (task.status === 'failed') throw new Error(task.error || '任务失败') } ElMessage.warning('任务仍在运行，可在全局任务中心查看') } catch (error) { ElMessage.error(`生成失败：${error.message}`) } finally { loading.value = false } }
...
const saveAsNote = async () => { const bookId = knowledgeBookIds.value[0]; try { await createKnowledgeNote({ book_id: bookId, title: (focus.value || '研究报告').slice(0,80), content: content.value, tags: ['研究报告'], origin: 'ai' }); ElMessage.success('已保存为独立知识笔记') } catch (error) { ElMessage.error(error.message) } }
const saveClaimsAsEvidence = async () => { const claims = activeReport.value?.claims || []; if (!claims.length) return ElMessage.warning('当前报告没有可保存的结构化主张'); let count = 0; for (const claim of claims) { const match = String(claim.source_refs?.[0] || '').match(/^B(\d+):/), bookId = match ? Number(match[1]) : knowledgeBookIds.value[0]; try { await createEvidenceCard({ book_id: bookId, title: claim.claim.slice(0,80), evidence_text: [`来源锚点：${(claim.source_refs || []).join('、') || '待补充'}`, claim.reason, claim.counterpoint].filter(Boolean).join('\n'), claim_text: claim.claim, tags: ['研究报告'], origin: 'ai', verification_status: claim.status || 'needs_review' }); count++ } catch { /* 单条失败不阻断其余保存 */ } } ElMessage.success(`已保存 ${count} 条证据卡片`) }
const sendToPptx = () => { sessionStorage.setItem('deckSelectedText', content.value); sessionStorage.setItem('deckSourceBookIds', JSON.stringify(knowledgeBookIds.value)); router.push({ path: '/literature-workbench', query: { from: 'research-report', bookId: knowledgeBookIds.value[0] } }) }
