      ...
      <main class="report-pane panel">
        ...
        <div class="research-brief">
          ...
          <div class="method-row writing-options">
            ...
            <label>Writing DNA（可选）
              <el-select v-model="dnaProfileId" clearable placeholder="不校准表达"><el-option v-for="profile in readyDnaProfiles" :key="profile.id" :label="`${profile.name} · v${profile.current_version}`" :value="profile.id" /></el-select>
            </label>
          </div>
          <label>研究问题
...
const writingStyle = ref('analytical_essay'), extensionLevel = ref('exploratory'), targetLength = ref(3000), dnaProfileId = ref(null)
const readyDnaProfiles = ref([])
const reportViolations = computed(() => {
  const v = activeReport.value?.selection?.ai_tone_violations
  if (!v || typeof v !== 'object') return null
  const items = [['“不是……而是……”句式', v.not_but], ['翻案式铺垫', v.fan_an], ['定语后置嫌疑', v.postposed_attr], ['例证复用', Array.isArray(v.repeated_example_pairs) ? v.repeated_example_pairs.map(pair => `${pair[0]} ≈ ${pair[1]}`) : null]]
  const hits = items.filter(([, list]) => Array.isArray(list) && list.length)
...
// knowledgeBookIds 每次由 setKnowledgeScope 整体替换，引用比较即可感知变化。
watch(knowledgeBookIds, () => { if (initialised) resetScope() })
onMounted(async () => {
