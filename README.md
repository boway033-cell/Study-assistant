...
- 长文献按章节分层取样并报告覆盖率；AI 提炼研究问题、论证脉络与关键证据。
- 先预览并人工编辑提纲，再渲染中文 deck；主张—来源审计会标出数字或定位不一致。
...
- 后端测试：96 项，覆盖解析、目录、文献工作台、批注和数据可靠性。
- 前端单元测试：7 项；另有真实 PDF 的 Playwright 浏览器回归。
- 当前版本：v1.1.0。项目处于持续迭代期，解析质量仍以“证据 + 置信度 + 人工复核”作为安全边界。
